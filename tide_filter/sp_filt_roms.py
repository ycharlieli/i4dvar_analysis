...
if __name__ == '__main__':
    Inp_files = ['/Volumes/TO_1/outputs_SCORRECTION/ocean_ecs_avg_'\
                  +str(i).rjust(4,'0')+'_t.nc' for i in range(1,5)]
...
#    count = 0
    for iy in range(ly):
        for ix in range(lx):
            mp_filt_var(Var_da,ix,iy,Cf,Nf,M)
...
#    Var_filt = Var_filt.reshape(shape)
    ...
    #    p.close()
    import matplotlib.pyplot as plt
    sp.io.savemat('testsp_filt_%s.mat'%(v),{'%s_std'%(v): Var_filt})    
    np.save('testsp_filt_%s'%(v),Var_filt)
